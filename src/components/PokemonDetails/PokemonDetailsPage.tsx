...
import {
	fetchAbilityDetailsById,
	fetchMoveDetailsById,
	fetchPokemonDetailsById,
	fetchPokemonEvolutionChainById,
} from '../../api';
...
export const PokemonDetailsPage = () => {
	...
	const evolutionChainId = pokemonDetails?.species.evolution_chain.url
		.split('/')
		.slice(-2)[0];
	...
	} = useFetch<PokemonEvolutionChain>(
		() => fetchPokemonEvolutionChainById(evolutionChainId as string),
		[pokemonDetails],
	);
	...
	const {
		...
		error: movesError,
	} = useFetch<LevelUpMoveWithDetails[]>(async () => {
		if (!pokemonDetails) return [];
		...
		return (
			<Wrapper className="h-auto!">
				<div className="mb-4">
					<Link
						to="/"
						className="h-10 inline-flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-gray-200 text-gray-700 rounded-sm w-full md:w-auto transition-colors border border-gray-200">
					...
					<div className="flex-1">
						<div className="mb-6">
							...
							<span className="text-gray-600">
								Min and Max values are calculated for Lv. 100
								Pokemon
							</span>
						</div>

						<StatsTable
							data={(pokemonDetails.details.stats || []).map(
								(stat) => {
									return {
										name: statNameMap[stat.stat.name],
										value: stat.base_stat,
										minMaxValues:
											calculateMinMaxStatValueAt100(
												stat.stat.name,
							...
							abilityDetails && (
								<DataTable
									rows={[
										...
										[
											'Height',
											`${pokemonDetails.details.height / 10} m`,
										],
										[
											'Weight',
											`${pokemonDetails.details.weight / 10} kg`,
										],
