...
export const PokemonList = () => {
	...
	const handleSearchChange = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		setFilteredPokemon(
			pokemonList?.results?.filter((pokemon: { name: string }) =>
	...
	if (pokemonList) {
		const totalResults = startedTyping
			? fileteredPokemon.length
			: pokemonList.results.length;
		const startIdx =
			totalResults === 0 ? 0 : (currentPage - 1) * maxResults + 1;
		...
		return (
			<div className="flex flex-col gap-2 overflow-hidden h-full  ">
				...
				<div className="flex flex-1 flex-wrap justify-center overflow-y-auto p-2 gap-2">
					{(startedTyping ? fileteredPokemon : pokemonList.results)
						?.length === 0 ? (
