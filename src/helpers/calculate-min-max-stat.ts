...
*/
export const calculateMinMaxStatValueAt100 = (statName: string, pokemonStats: Stat[]): { min: number; max: number; } => {


    let maxEV = 63; // max EV for a stat
    let maxIV = 31; // max IV for a stat

    const baseStat = pokemonStats.find(stat => stat.stat.name === statName)?.base_stat || 0;
